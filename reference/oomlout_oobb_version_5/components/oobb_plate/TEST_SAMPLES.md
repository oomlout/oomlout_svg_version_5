...
### Sample 1: `test_1.png`
- Intent: plain OOBB plate.
- preview_rot: `[55, 0, 25]`
- kwargs: `{"pos":[0,0,0],"width":3,"height":2,"depth":3,"zz":"bottom","holes":false}`
...
### Sample 2: `test_2.png`
- Intent: plate with hole pattern.
- preview_rot: `[55, 0, 25]`
- kwargs: `{"pos":[0,0,0],"width":3,"height":2,"depth":3,"zz":"bottom","holes":true}`
- Implementation rule: keep the sample values exactly as written. If `kwargs` already contains `rot`, do not replace it; use `preview_rot` only as the outer rotation wrapper for the final rendered scene.
...
- Notes: use the listed `preview_rot` so the hole pattern and thickness are both visible.
