...
### Sample 1: `test_1.png`
- Intent: inset profile one direction.
- preview_rot: `[55, 0, 25]`
- kwargs: `{"type":"positive","size":[24,14,6],"r":4,"inset":2,"rotY":0,"pos":[0,0,0]}`
...
### Sample 2: `test_2.png`
- Intent: same profile flipped with `rotY=180`.
- preview_rot: `[55, 0, 25]`
- kwargs: `{"type":"positive","size":[24,14,6],"r":4,"inset":2,"rotY":180,"pos":[0,0,0]}`
- Implementation rule: keep the sample values exactly as written. If `kwargs` already contains `rot`, do not replace it; use `preview_rot` only as the outer rotation wrapper for the final rendered scene.
...
- Notes: use the listed `preview_rot` so the inset direction change is visible.
