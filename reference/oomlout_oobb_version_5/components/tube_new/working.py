...
def describe():
    ...
    d = {}
    d["name"] = "tube_new"
    d["name_long"] = "OPSC Composite Shapes: Tube New"
    d["description"] = "Legacy opsc tapered tube shape migrated into the component system."
...
def render(params):
    ...
    p2.setdefault("wall_thickness", 1)
    if "r1" in p2:
        if "h" not in p2:
            if "height" in p2:
                p2["h"] = p2["height"]
        ...
        p3 = copy.deepcopy(p2)
        p3["type"] = "positive"
        p3["r1"] = p3["r1"] + p3["wall_thickness"]
        p3["r2"] = p3["r2"] + p3["wall_thickness"]
