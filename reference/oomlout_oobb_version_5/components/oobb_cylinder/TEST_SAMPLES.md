...
### Sample 2: `test_2.png`
- Intent: explicit-radius cylinder.
- preview_rot: `[45, 0, 25]`
- kwargs: `{"pos":[0,0,0],"depth":18,"radius":6,"zz":"center"}`
- Implementation rule: keep the sample values exactly as written. If `kwargs` already contains `rot`, do not replace it; use `preview_rot` only as the outer rotation wrapper for the final rendered scene.
...
- Notes: if `radius_name` handling expects project variable lookup, use a known valid key in implementation.
