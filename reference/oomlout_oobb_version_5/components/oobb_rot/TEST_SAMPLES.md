...
Apply these instructions exactly in `working.py` for this folder. This component is a helper, so the test must render a companion object instead of trying to render the helper return value by itself.
...
### Special rule for this helper

- Call `action(**helper_kwargs)` first.
- Use the returned rotation list as `companion_geometry_kwargs["rot"]`.
- Build the companion geometry scene with `oobb.oobb_easy(**companion_geometry_kwargs)`.
- Only after that, apply the outer `preview_rot` wrapper if the sample says to do so.
...
### Sample 1: `test_1.png`
- Intent: visualize helper output using a companion cube.
- preview_rot: `[0, 0, 0]`
- helper_kwargs: `{"rot":[30,20,10]}`
- companion_geometry_kwargs: `{"shape":"cube","type":"positive","size":[24,18,12],"pos":[0,0,0]}`
...
- Output files: `test_1.png`
- Notes: this helper is not standalone geometry. The test should call the helper, apply the returned rotation to the companion cube, and render that scene.
