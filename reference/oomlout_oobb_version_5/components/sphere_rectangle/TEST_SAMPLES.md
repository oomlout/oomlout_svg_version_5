...
### Sample 1: `test_1.png`
- Intent: sphere-swept rounded block.
- preview_rot: `[45, 0, 25]`
- kwargs: `{"type":"positive","size":[24,16,10],"r":3,"pos":[0,0,0]}`
...
### Sample 2: `test_2.png`
- Intent: larger rounded block variant.
- preview_rot: `[45, 0, 25]`
- kwargs: `{"type":"positive","size":[32,20,12],"r":4,"pos":[0,0,0]}`
- Implementation rule: keep the sample values exactly as written. If `kwargs` already contains `rot`, do not replace it; use `preview_rot` only as the outer rotation wrapper for the final rendered scene.
...
- Notes: use the listed `preview_rot` so both end rounding and side surfaces are visible.
