...
### Sample 1: `test_1.png`
- Intent: simple inline cube module.
- preview_rot: `[35, 0, 25]`
- kwargs: `{"type":"positive","source":"module main(){cube([12,12,12], center=true);}","module":"main","pos":[0,0,0]}`
...
### Sample 2: `test_2.png`
- Intent: inline bracket-like difference shape.
- preview_rot: `[35, 0, 25]`
- kwargs: `{"type":"positive","source":"module main(){difference(){cube([20,12,6], center=true); translate([0,0,0]) cube([10,6,8], center=true);}}","module":"main","pos":[0,0,0]}`
- Implementation rule: keep the sample values exactly as written. If `kwargs` already contains `rot`, do not replace it; use `preview_rot` only as the outer rotation wrapper for the final rendered scene.
...
- Notes: keep the second sample because it proves more than trivial source passthrough.
