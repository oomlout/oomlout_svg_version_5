...
### Sample 1: `test_1.png`
- Intent: compact open tray.
- preview_rot: `[45, 0, 25]`
- kwargs: `{"type":"positive","size":[30,20,10],"radius":4,"wall_thickness":1.2,"pos":[0,0,0]}`
...
### Sample 2: `test_2.png`
- Intent: wider tray variant.
- preview_rot: `[45, 0, 25]`
- kwargs: `{"type":"positive","size":[40,24,12],"radius":5,"wall_thickness":1.5,"pos":[0,0,0]}`
- Implementation rule: keep the sample values exactly as written. If `kwargs` already contains `rot`, do not replace it; use `preview_rot` only as the outer rotation wrapper for the final rendered scene.
...
- Notes: use the listed `preview_rot` so the open cavity is clearly visible.
